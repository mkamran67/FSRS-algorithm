...
describe("FSRS", () => {
	...
	test("should create an empty card and review it for the first time", () => {
		...
		// 2. After the user views the card, schedule it
		const scheduling = fsrs.schedule(card, Rating.Good, now);

		// 3. should have 2 reps since these are hypotheticals (next rep included)
	...
	xtest("should handle review card", () => {
	...
	xtest("should calculate retrievability", () => {
		let card = fsrs.createEmptyCard(now);
		const scheduling = fsrs.schedule(card, Rating.Good, now);
	...
	xtest("should update parameters", () => {
